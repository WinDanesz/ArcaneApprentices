package com.windanesz.arcaneapprentices.misc;
 ...
 * attempted.<br>
 * <br>
 * Adding a new forfeit is as simple as calling {@link ForfeitAA#add(Tier, Element, ForfeitAA)} and supplying the
 * {@code Forfeit} instance along with a tier and element to associate it with. To create a forfeit, you may extend
 * this class and instantiate it, or use {@link ForfeitAA#create(ResourceLocation, BiConsumer)} to concisely define the
...
public abstract class ForfeitAA {

	private static final ListMultimap<Pair<Tier, Element>, ForfeitAA> forfeits = ArrayListMultimap.create();
	...
	public ForfeitAA(ResourceLocation name) {
		this.name = name;
		this.sound = WizardrySounds.createSound("forfeit." + name.getPath());
	}

	public abstract void apply(World world, EntityLivingBase player);
	 ...
	 * @see ForfeitAA#getMessageForWand()
	 * @see ForfeitAA#getMessageForScroll()
	 */
	...
	/**
	 * Wrapper for {@link ForfeitAA#getMessage(ITextComponent)} with {@code implementName} set to the lang file key
	...
	public static void add(Tier tier, Element element, ForfeitAA forfeit) {
		forfeits.put(Pair.of(tier, element), forfeit);
	}

	public static ForfeitAA getRandomForfeit(Random random, Tier tier, Element element) {
		float f = random.nextFloat();
		if (f < TIER_CHANGE_CHANCE) tier = tier.previous();
		else if (f > 1 - TIER_CHANGE_CHANCE) tier = tier.next();
		List<ForfeitAA> matches = forfeits.get(Pair.of(tier, element));
		if (matches.isEmpty()) {
			return null;
		}
	...
	public static Collection<ForfeitAA> getForfeits() {
	 ...
	 */
	public static ForfeitAA create(ResourceLocation name, BiConsumer<World, EntityLivingBase> effect) {
		return new ForfeitAA(name) {
			@Override
			public void apply(World world, EntityLivingBase player) {
	...
	/**
	 * Internal wrapper for {@link ForfeitAA#create(ResourceLocation, BiConsumer)} so I don't have to put wizardry's
	 * mod ID in every time.
	 */
	private static ForfeitAA create(String name, BiConsumer<World, EntityLivingBase> effect) {
	...
	public static void register() {
		...
		add(Tier.MASTER, Element.FIRE, create("meteors", (w, p) -> {
			if (!w.isRemote) for (int i = 0; i < 5; i++)
				w.spawnEntity(new EntityMeteor(w, p.posX + w.rand.nextDouble() * 16 - 8,
		...
		add(Tier.NOVICE, Element.SORCERY, create("spill_inventory", (w, p) -> {
			if (p instanceof EntityPlayer) {
				...
				for (int i = 0; i < ep.inventory.mainInventory.size(); i++) {
					ItemStack stack = ep.inventory.mainInventory.get(i);
					if (!stack.isEmpty()) {
						ep.dropItem(stack, true, false);
						ep.inventory.mainInventory.set(i, ItemStack.EMPTY);
					}
				}
			}
		}));
		...
		add(Tier.NOVICE, Element.HEALING, create("spill_armour", (w, p) -> {
			if (p instanceof EntityPlayer) {
				EntityPlayer ep = (EntityPlayer) p;
				for (int i = 0; i < ep.inventory.armorInventory.size(); i++) {
					ItemStack stack = ep.inventory.armorInventory.get(i);
					if (!stack.isEmpty()) {
						ep.dropItem(stack, true, false);
						ep.inventory.armorInventory.set(i, ItemStack.EMPTY);
					}
				}
			}
		}));
