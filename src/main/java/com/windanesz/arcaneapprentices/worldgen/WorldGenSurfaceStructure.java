...
/**
 * Base class for all Arcane Apprentices' above-ground structures, which handles various shared functions such as calculating
 ...
 *
 * @author Electroblob (original), WinDanesz (adapted for Arcane Apprentices)
 * @since Arcane Apprentices 1.0
 */
public abstract class WorldGenSurfaceStructure extends WorldGenWizardryStructure {
	...
	protected BlockPos attemptPosition(Template template, PlacementSettings settings, Random random, World world,
		...
		// Safety check: if the structure has no area, reject it
	...
	protected void postGenerate(Random random, World world, PlacementSettings settings){
		if(!Wizardry.settings.fastWorldgen) removeFloatingTrees(world, settings.getBoundingBox(), random, null);
	}

	/** 
	 * Finds and removes any floating bits of tree in and above the given structure bounding box.
	 * @param structureBlocks Optional set of block positions that are part of the structure and should not be removed
	 */
	protected static void removeFloatingTrees(World world, StructureBoundingBox boundingBox, Random random, Set<BlockPos> structureBlocks){
		...
		while(changed && y < world.getHeight()){ // I do hope the trees don't reach the world height...
			...
			for(int x = boundingBox.minX; x <= boundingBox.maxX; x++){
				...
				if(block instanceof BlockLog){
					// Skip if this block is part of the structure
					if(structureBlocks != null && structureBlocks.contains(pos)) continue;
